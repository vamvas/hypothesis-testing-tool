from hypothesis_testing_tool.compute_stats.one_sample_statistics import OneSampleMean
import pytest


def test_one_sample_t_test_returns_correct_t_statistic_and_p_value_for_alternative_of_two_sided():

    t_test = OneSampleMean(data=[1, 2, 5, 8, 10], null_population_mean=3.5).t_test_results
...
def test_one_sample_t_test_returns_correct_t_statistic_and_p_value_for_alternative_of_lower():
    ...
    t_test = OneSampleMean(data=[1, 2, 5, 8, 10], null_population_mean=3.5, alternative="less").t_test_results
...
def test_one_sample_t_test_returns_correct_t_statistic_and_p_value_for_alternative_of_higher():
    ...
    t_test = OneSampleMean(data=[1, 2, 5, 8, 10], null_population_mean=3.5, alternative="greater").t_test_results
...
def test_one_sample_confidence_interval_has_correct_lower_and_upper_bound():
    ...
    confidence_interval = OneSampleMean(data=[1, 2, 5, 8, 10], null_population_mean=3.5).calculate_ci()
...
def test_one_sample_confidence_interval_has_correct_lower_and_upper_bound_for_alpha_different_than_default():
    ...
    confidence_interval = OneSampleMean(data=[1, 2, 5, 8, 10], null_population_mean=3.5).calculate_ci(alpha=0.01)
...
def test_one_sample_t_test_raises_error_when_input_has_less_than_two_elements(
    ...
    with pytest.raises(
        ...
        OneSampleMean(input_data, null_population_mean=3.5)
